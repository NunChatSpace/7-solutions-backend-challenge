...
func TestGetUserByID(t *testing.T) {
	...
	userService := userservices.NewUserService(mockRepo)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result != nil && *result.ID != *tt.expected.ID {
				t.Errorf("expected user ID %d, got %d", *tt.expected.ID, *result.ID)
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result != nil && *result[0].ID != *tt.expected[0].ID {
				t.Errorf("expected user ID %d, got %d", *tt.expected[0].ID, *result[0].ID)
			}
